...
class BioWorker @Inject constructor(
    ...
    override fun doWork(): Result {
        ...
        userCollection.document(uid).get().addOnSuccessListener {
            if (it != null && it.exists()) {
                ...
                when (type) {
                    SelectType.FLIRT -> {
                        userFlirtCollection.document(country).collection(country)
                            .document(uid)
                            .update(FirestoreServiceImpl.DESCRIPTION_FIELD, text)
                    }

                    SelectType.MARRIAGE -> {
                        userMarriageCollection.document(country).collection(country)
                            .document(uid)
                            .update(FirestoreServiceImpl.DESCRIPTION_FIELD, text)
                    }
                    ...
                    SelectType.LANGUAGE_PRACTICE -> {
                        ...
                        userLanguagePracticeCollection.document(motherTongue)
                            .collection(motherTongue).document(uid)
                            .update(FirestoreServiceImpl.DESCRIPTION_FIELD, text)
                    }

                    SelectType.HOTEL -> {
                        hotelCollection.document(country)
                            .collection(FirestoreServiceImpl.HOTEL_COLLECTION)
                            .document(uid)
                            .update(FirestoreServiceImpl.DESCRIPTION_FIELD, text)
                    }

                    SelectType.RESTAURANT -> {
                        restaurantCollection.document(country)
                            .collection(FirestoreServiceImpl.RESTAURANT_COLLECTION)
                            .document(uid)
                            .update(FirestoreServiceImpl.DESCRIPTION_FIELD, text)
                    }

                    SelectType.CAFE -> {
                        cafeCollection.document(country)
                            .collection(FirestoreServiceImpl.CAFE_COLLECTION)
                            .document(uid)
                            .update(FirestoreServiceImpl.DESCRIPTION_FIELD, text)
                    }
