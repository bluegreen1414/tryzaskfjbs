...
class LanguagePracticeWorker @Inject constructor(
    ...
    override fun doWork(): Result {
        val randomid = inputData.getString(FirestoreServiceImpl.HOTEL_COLLECTION) ?: return Result.failure()
        val uid = inputData.getString(FirestoreServiceImpl.RESTAURANT_COLLECTION) ?: return Result.failure()
        val photoFile = inputData.getString(FirestoreServiceImpl.CAFE_COLLECTION) ?: return Result.failure()
                            ...
                            userCollection.document(uid).update(FirestoreServiceImpl.LEARN_LANGUAGE_FIELD, learnLanguage).addOnSuccessListener {
                                userCollection.document(uid).update(FirestoreServiceImpl.MOTHER_TONGUE_FIELD, motherTongue).addOnSuccessListener {
                                    userLanguagePracticeCollection.document(motherTongue)
                                        .collection(motherTongue).document(uid).set(
                                        UserLP(
