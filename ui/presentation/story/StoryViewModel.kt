...
class StoryViewModel @Inject constructor(
    ...
    var media by mutableStateOf<List<StoryItem>>(emptyList())
    ...
    private lateinit var preloadManager: PreloadManagerWrapperStory
    ...
    fun getStories(id: String){
        launchCatching {
            media = emptyList<StoryItem>()
            val newList = mutableListOf<StoryItem>()

            ff.collection(FirestoreServiceImpl.STORY_COLLECTION)
                .document(id).collection(FirestoreServiceImpl.STORY_COLLECTION).get().addOnCompleteListener {
                if (it.isSuccessful && it.result != null) {
                    val x = it.result.toObjects(StoryItem::class.java)
    ...
    private fun initPreloadManager(
        ...
        preloadManager =
            PreloadManagerWrapperStory.build(
    ...
    private val timeline = mutableStateOf<StoryItem?>(null)
    fun onReportClick(newValue: StoryItem) {
    ...
    //fun markVideoAsWatched(video: ExploreVideo) {
    //  launchCatching {
    //watchedVideoDao.insert(WatchedVideo(video.id))
    //}
    //}
